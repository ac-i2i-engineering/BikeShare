...
const COMM = {
  ...
  handleCommunication: (commID, context) => {
    ...
    Logger.log(
      `Processing communication ${commID} with context:${JSON.stringify(context)}`,
    );
    const results = [];

    if (
      comm.notifyUser &&
      CACHED_SETTINGS.VALUES.NOTIFICATION_SETTINGS.ENABLE_USER_NOTIFICATIONS
    ) {
      Logger.log(
        `Preparing user notification to ${context.userEmail} for ${commID}`,
      );

      // Fill placeholders with validation
      const subjectResult = COMM.fillPlaceholders(
        comm.notifyUser.subject,
        context,
        commID,
      );
      const bodyResult = COMM.fillPlaceholders(
        comm.notifyUser.body,
        context,
        commID,
      );
      ...
      if (subjectResult.hasErrors || bodyResult.hasErrors) {
        Logger.log(
          `❌ Cannot send user notification for ${commID} - missing data: ${[...new Set([...(subjectResult.missingKeys || []), ...(bodyResult.missingKeys || [])])].join(", ")}`,
        );
        results.push({
          success: false,
          error: "Missing placeholder data",
          commID,
        });
      } else {
      ...
      Logger.log(
        `Skipping user notification - notifyUser: ${!!comm.notifyUser}, enabled: ${CACHED_SETTINGS.VALUES.NOTIFICATION_SETTINGS.ENABLE_USER_NOTIFICATIONS}`,
      );
    }

    if (
      comm.notifyAdmin &&
      CACHED_SETTINGS.VALUES.NOTIFICATION_SETTINGS.ENABLE_ADMIN_NOTIFICATIONS
    ) {
      Logger.log(`Preparing admin notification for ${commID}`);

      const subjectResult = COMM.fillPlaceholders(
        comm.notifyAdmin.subject,
        context,
        commID,
      );
      const bodyResult = COMM.fillPlaceholders(
        comm.notifyAdmin.body,
        context,
        commID,
      );
      ...
      Logger.log(
        `Skipping admin notification - notifyAdmin: ${!!comm.notifyAdmin}, enabled: ${CACHED_SETTINGS.VALUES.NOTIFICATION_SETTINGS.ENABLE_ADMIN_NOTIFICATIONS}`,
      );
    }

    if (
      comm.notifyDeveloper &&
      CACHED_SETTINGS.VALUES.NOTIFICATION_SETTINGS.ENABLE_DEV_NOTIFICATIONS
    ) {
      Logger.log(`Preparing developer notification for ${commID}`);

      const subjectResult = COMM.fillPlaceholders(
        comm.notifyDeveloper.subject,
        context,
        commID,
      );
      const bodyResult = COMM.fillPlaceholders(
        comm.notifyDeveloper.body,
        context,
        commID,
      );
      ...
      Logger.log(
        `Skipping developer notification - notifyDeveloper: ${!!comm.notifyDeveloper}, enabled: ${CACHED_SETTINGS.VALUES.NOTIFICATION_SETTINGS.ENABLE_DEV_NOTIFICATIONS}`,
      );
    }

    if (comm.markEntry && context.range) {
      Logger.log(
        `Marking entry per admin config - Code: ${commID}, Color: ${comm.markEntry.bgColor}, Note: ${comm.markEntry.note}`,
      );

      const noteResult = COMM.fillPlaceholders(
        comm.markEntry.note,
        context,
        commID,
      );
      ...
      Logger.log(
        `Skipping entry marking - markEntry: ${comm.markEntry ? "configured as null" : "not configured"}, range: ${!!context.range}`,
      );
    }
  ...
  notifyUser: (
    ...
    let bodyStr = "";
    const ccList = [];
    if (friendEmail) ccList.push(friendEmail);
    if (String(commID).startsWith("ERR_"))
      ccList.push(CACHED_SETTINGS.VALUES.ADMIN_EMAIL);
    ...
    try {
      ...
      if (subjectStr.includes("[MISSING:") || bodyStr.includes("[MISSING:")) {
        Logger.log(
          `❌ Blocking malformed email to ${userEmail} for ${commID} - contains missing placeholders`,
        );
      ...
      GmailApp.sendEmail(
        ...
        subjectStr,
        `Hi,\n\n${bodyStr}\n\nThank you,\nBike Share Team`,
        { from: CACHED_SETTINGS.VALUES.ORG_EMAIL, cc: ccList.join(",") },
      );
      Logger.log(`✅ Email sent successfully to ${userEmail} for ${commID}`);
      return { success: true, recipient: userEmail };
    } catch (error) {
      Logger.log(
        `❌ Failed to send email to ${userEmail} from ${CACHED_SETTINGS.VALUES.ORG_EMAIL} for ${commID}: ${error.message}`,
      );
      ...
      try {
        ...
        Logger.log(
          `❌ Also failed to notify admin of email failure: ${adminError.message}`,
        );
      }
  ...
  getCommunication: (commID) =>
    CACHED_SETTINGS.VALUES.COMM_CODES[commID] || null,
  ...
  fillPlaceholders: (template, context, commID = "unknown") => {
    ...
    const result = template.replace(/{{(.*?)}}/g, (match, key) => {
      ...
      if (value === undefined || value === null || value === "") {
        ...
        Logger.log(
          `❌ Missing placeholder data for '${trimmedKey}' in ${commID}. available: ${Object.keys(context).join(", ")}`,
        );
        //DEBUG
      ...
      Logger.log(
        `✅ Filled placeholder '${trimmedKey}' with '${value}' in ${commID}`,
      );
      return String(value);
    });
    ...
    if (hasErrors) {
      Logger.log(
        `⚠️ Template filling errors in ${commID}. Missing keys: ${missingKeys.join(", ")}`,
      );
    }

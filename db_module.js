const DB = {
  ...
  sortByColumn: (sheetName, spreadsheetId = null) => {
    try {
      ...
      try {
        ...
        Logger.log(
          `Error getting sheet config for '${sheetName}': ${error.message}`
        );
      }
      ...
      Logger.log(
        `Sorted '${sheetName}' by column ${sortColumn} (${
          ascending ? "asc" : "desc"
        }) - ${numRows - 1} rows`
      );
    } catch (error) {
  ...
  markEntry: (range, bgColor = null, note = null) => {
    ...
    try {
      ...
      Logger.log(
        `DB.markEntry: Successfully marked range with color: ${bgColor}, note: ${note}`
      );
    } catch (error) {
  ...
  batchUpdate: (operations, spreadsheetId = null) => {
    ...
    operations.forEach((op) => {
      ...
      operationsBySheet[op.sheetName].sheet = DB.getSheet(
        op.sheetName,
        spreadsheetId
      );
    ...
    Object.keys(operationsBySheet).forEach((sheetName) => {
      ...
      try {
        ...
        if (sheetOps.updates.length > 0) {
          ...
          Logger.log(
            `Successfully batch updated ${sheetOps.updates.length} rows`
          );
        ...
        if (sheetOps.appends.length > 0) {
          Logger.log(
            `Batch appending ${sheetOps.appends.length} rows to sheet: ${sheetName}`
          );
          ...
          Logger.log(
            `Appending to rows ${startRow} to ${
              startRow + sheetOps.appends.length - 1
            }`
          );
        ...
        Logger.log(
          `Batch operation failed for ${sheetName}, falling back to individual operations: ${error.message}`
        );
        ...
        sheetOps.updates.forEach((update) => {
          try {
            sheet
              .getRange(update.rowIndex, 1, 1, update.values.length)
  ...
  resetDatabase: (spreadsheetId = null) => {
    Logger.log(
      "🔄 Starting database reset (soft reset - clearing content only)"
    );

    if (
      !CACHED_SETTINGS.VALUES.ENABLE_FORCED_RESET ||
      !CACHED_SETTINGS.VALUES.DEBUG_MODE
    ) {
    ...
    // Log which spreadsheet we're working with
    if (spreadsheetId) {
      Logger.log(
        `📊 Resetting database in specific spreadsheet: ${spreadsheetId}`
      );
    } else {
      Logger.log(`📊 Resetting database in active/cached spreadsheet`);
    }

    Logger.log(
      `📋 Processing ${
        Object.keys(CACHED_SETTINGS.VALUES.SHEETS).length
      } sheets for reset`
    );
    ...
    for (const key in CACHED_SETTINGS.VALUES.SHEETS) {
      try {
        ...
        const sheet = DB.getSheet(sheetConfig.NAME, spreadsheetId); // Pass spreadsheetId

        if (!sheet) {
          Logger.log(
            `⚠️ Sheet '${sheetConfig.NAME}' not found for key '${key}' in ${
              spreadsheetId ? "specified spreadsheet" : "active spreadsheet"
            }`
          );
          errorCount++;
          continue;
        ...
        Logger.log(
          `✅ Successfully reset sheet '${key}' (range: ${sheetConfig.RESET_RANGE})`
        );
        successCount++;
      } catch (error) {
    ...
    Logger.log(
      `🎯 Reset completed - Success: ${successCount}, Errors: ${errorCount}`
    );

    if (errorCount > 0) {
      Logger.log(
        `⚠️ Reset completed with ${errorCount} errors. Check logs above for details.`
      );
    } else {
  ...
  // Hard reset database by deleting rows in all configured sheets
  hardResetDatabase: (spreadsheetId = null) => {
    Logger.log("🔄 Starting hard database reset (deleting rows)");
    ...
    if (
      ...
      const error = "Auto reset is disabled or debug mode is off.";
      Logger.log(`❌ Hard reset failed: ${error}`);
      throw new Error(error);
    }
    ...
    if (spreadsheetId) {
      Logger.log(
        `📊 Hard resetting database in specific spreadsheet: ${spreadsheetId}`
      );
    } else {
      Logger.log(`📊 Hard resetting database in active/cached spreadsheet`);
    }
    ...
    for (const key in CACHED_SETTINGS.VALUES.SHEETS) {
      if (key === "BIKES_STATUS") {
        Logger.log(`⏭️ Skipping sheet: ${key} (protected from hard reset)`);
        skippedCount++;
        continue;
      }

      try {
        Logger.log(`🗑️ Hard resetting sheet: ${key}`);
        const sheetConfig = CACHED_SETTINGS.VALUES.SHEETS[key];
        const sheet = DB.getSheet(sheetConfig.NAME, spreadsheetId); // Pass spreadsheetId
        ...
        const lastRow = sheet.getLastRow();
        const startRow = 2; // Keep header row
        const numRows = lastRow - 1;

        if (numRows < 1) {
          Logger.log(`⚪ Sheet ${key} is already empty, no rows to delete`);
        ...
        try {
          ...
          Logger.log(
            `✅ Successfully deleted rows ${startRow}-${lastRow} in ${key}`
          );
          successCount++;
        } catch (deleteError) {
          Logger.log(
            `❌ Error deleting rows ${startRow}-${lastRow} in ${key}: ${deleteError.message}`
          );
          errorCount++;
        }
      } catch (error) {
        Logger.log(`❌ Error processing sheet '${key}': ${error.message}`);
        errorCount++;
      }
    }

    Logger.log(
      `🎯 Hard reset completed - Success: ${successCount}, Errors: ${errorCount}, Skipped: ${skippedCount}`
    );

    if (errorCount > 0) {
      Logger.log(
        `⚠️ Hard reset completed with ${errorCount} errors. Check logs above for details.`
      );
    } else {
      Logger.log("✨ All applicable sheets successfully hard reset!");
    }
  },
  ...
  updateRow: (sheetName, rowIndex, values) => {
    try {
      ...
      Logger.log(
        `Error updating row ${rowIndex} in '${sheetName}': ${error.message}`
      );
      throw error;
    }
  },
